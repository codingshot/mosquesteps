# MosqueSteps — Marketing Folder

This folder holds all marketing copy, research, and campaign materials. Use it for content, SEO, partnerships, and PR.

## Contents

| File | Purpose |
|------|--------|
| **brand-guidelines.md** | Voice, colors, typography, logo, Islamic content standards |
| **copy.md** | Taglines, hero copy, feature headlines, SEO keywords, AI/LLM summary |
| **social-ads.md** | Ready-to-use ad copy (short/long), email subject lines, UGC prompts, AI summary |
| **campaign-ideas.md** | Campaign concepts (Mosque Next Door, Walk With Me, Fajr Challenge, etc.) |
| **content-calendar.md** | Monthly themes, content types, blog pipeline, social templates |
| **content-hooks.md** | One-liners, questions, stats, story angles, CTAs for posts and reels |
| **founder-announcement-hadith.md** | Founder announcements: X threads, Sunnah outline, features to test, open source; hadith-based for each social |
| **keyword-clusters.md** | SEO keyword groups by intent (product, rewards, prayer, health, etc.) |
| **testimonials-templates.md** | How to ask for quotes, placeholder testimonials, permission checklist |
| **influencer-brief.md** | One-pager for influencers and partners; what we offer and ask |
| **press-kit.md** | Boilerplate, quotes, stats, media angles, contact, do-not-claim |
| **email-templates.md** | Welcome, re-engagement, Ramadan, feature highlight, share-with-friend |
| **elderly-outreach.md** | Messaging and channels for elderly users and families |
| **community-mosque-benefits.md** | Research and messaging for mosque communities and walking groups |
| **exercise-health-research.md** | Health stats and sources for marketing and blog |
| **habit-psychology.md** | Habit formation research and marketing applications |
| **improvements-roadmap.md** | Product roadmap, metrics, technical debt |

## Quick links

- **Need a tagline or headline?** → copy.md, content-hooks.md  
- **Founder announcement / hadith-based posts?** → founder-announcement-hadith.md (X threads, Sunnah outline, features to test, open source)  
- **Running an ad?** → social-ads.md, keyword-clusters.md  
- **Planning content?** → content-calendar.md, campaign-ideas.md  
- **Talking to press or partners?** → press-kit.md, influencer-brief.md  
- **Sending email?** → email-templates.md  
- **Collecting testimonials?** → testimonials-templates.md  
- **Staying on brand?** → brand-guidelines.md  

## Root: IMPROVEMENT_PROMPTS.md

In the **project root** (not this folder), see **IMPROVEMENT_PROMPTS.md** for copy-paste prompts to improve the app, marketing, SEO, tests, and more.
