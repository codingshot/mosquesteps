...
export function formatDirection(instruction: string): string {
  ...
  if (actionPart.startsWith("roundabout")) return (actionPart.includes("left") ? "Enter roundabout, take exit left" : actionPart.includes("right") ? "Enter roundabout, take exit right" : "Enter roundabout") + street;
...
export function formatDistanceForStep(distanceM: number): string {
